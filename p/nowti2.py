            ...
            if d == idx:
                ...
                for k in range(i,la):
                    if "</table>" in all[k]:
                        ...
                        print(title_load(tin[sid]+" OP("+indata(all[k][all[k].rfind("「")+1:all[k].rfind("」")])+")"))
                        mtitle.append(title_load(tin[sid]+" OP("+indata(all[k][all[k].rfind("「")+1:all[k].rfind("」")])+")"))
                    elif "ED：" in all[k]:
                        print(title_load(tin[sid]+" ED("+indata(all[k][all[k].rfind("「")+1:all[k].rfind("」")])+")"))
                        mtitle.append(title_load(tin[sid]+" ED("+indata(all[k][all[k].rfind("「")+1:all[k].rfind("」")])+")"))
                break
    if end:
        break
